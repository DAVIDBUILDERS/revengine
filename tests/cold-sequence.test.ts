...
describe('hosted cold-sequence model', () => {
  it('does not invent a model off Vercel when no gateway key is set', () => {
    const config = coldSequenceModelConfig({});
    expect(config.usable).toBe(false);
    expect(hostedColdSequenceModel('workspace', 'live', {})).toBeUndefined();
    expect(hostedColdSequenceModel('workspace', 'fixture', { VERCEL: '1' })).toBeUndefined();
  });

  it('uses the hosted gateway on Vercel without requiring the four model env vars', () => {
    const config = coldSequenceModelConfig({ VERCEL: '1' });
    ...
    expect(hostedColdSequenceModel('workspace', 'live', { VERCEL: '1' })).toBeDefined();
  });
