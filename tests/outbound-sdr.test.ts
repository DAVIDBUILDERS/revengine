...
describe('Outbound Email SDR — Instantly, autonomous, no lead gen', () => {
  ...
  it('asks for a meeting link, writes copy from company facts, and starts without per-email approval', async () => {
    ...
    expect(state.outboundSdr?.sequence[0]?.subject).toMatch(/after the deck/i);
    ...
    expect(state.outboundSdr?.sequence[0]?.body).toMatch(/next 30 days/i);
    ...
    expect(state.outboundSdr?.sequence[1]?.body).toMatch(/who owns the first change|who can say yes or no/i);
