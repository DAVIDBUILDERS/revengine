...
export function createFixtureState(workspaceKey='david'):EngineState {
  ...
  fixtures.forEach(([reference,name,status,suppressed,stale],index)=>{
    ...
    if(reference==='P-1001'){
      state.timeline.push({id:stableId(`${workspaceKey}:message:out:${reference}`),workspaceId,opportunityId,at:'2026-09-10T12:10:00.000Z',kind:'message_out',title:`${reference} · follow-up`,detail:'Checking whether the existing proposal is still the right next step. No new terms.',actor:'david',evidence:[source]});
      state.timeline.push({id:stableId(`${workspaceKey}:message:in:${reference}`),workspaceId,opportunityId,at:'2026-09-10T13:05:00.000Z',kind:'message_in',title:`${name} · reply`,detail:'Thanks — we are reviewing internally. Please hold further automated follow-up.',actor:'human',evidence:[source]});
    }
  });
