...
import {parseCsvImport,forecastCases,parseLeadCsv,mappedLeadRow,writeOutboundSequence,companyFromSnapshot,emptyOutboundSdr,outboundSequenceCompany,slotAgentIds,technicalSeoAnswers,emptyTechnicalSeo} from '../../../packages/domain/src/index';
...
import {hostedColdSequenceModel} from '../../../packages/orchestration/src/cold-sequence';
...
export async function operationalCommand(command:C.Command,requested:string|null):Promise<C.CommandResult>{
 ...
 switch(command.type){
  ...
  case 'human_note':throw new HttpError(403,'FIXTURE_ONLY','Owner notes in the conversation log are only available in isolated fixture sessions.');
  ...
  case 'generate_outbound_sequence':{const company=outboundSequenceCompany(current);const bookingUrl=current.outboundSdr?.bookingUrl??'';if(current.workspace.mode==='fixture'){const drafted=await writeOutboundSequence(company,bookingUrl);await rpc('save_outbound_sequence',{p_workspace:workspaceId,p_sequence:drafted.sequence});message='Three cold emails drafted for this service. Instantly will send them; no copy-out to another mail tool.';break;}void process.env.VERCEL_OIDC_TOKEN;void process.env.AI_GATEWAY_API_KEY;const model=hostedColdSequenceModel(workspaceId,current.workspace.mode);if(!model)throw new HttpError(503,'MODEL_ACCESS_MISSING','DAVID could not reach the model that writes these emails. Try again.');const drafted=await writeOutboundSequence(company,bookingUrl,model,{requireModel:true});await rpc('save_outbound_sequence',{p_workspace:workspaceId,p_sequence:drafted.sequence});message='Three cold emails drafted from your services with the cold-outreach prompt. Instantly will send them; no copy-out to another mail tool.';break;}
