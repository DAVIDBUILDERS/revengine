...
export async function operationalCommand(command:C.Command,requested:string|null):Promise<C.CommandResult>{
 ...
 switch(command.type){
  ...
  case 'generate_outbound_sequence':{const company=outboundSequenceCompany(current);const drafted=await writeOutboundSequence(company,current.outboundSdr?.bookingUrl??'',hostedColdSequenceModel(workspaceId,current.workspace.mode));await rpc('save_outbound_sequence',{p_workspace:workspaceId,p_sequence:drafted.sequence});message=drafted.source==='model'?'Three cold emails drafted from your topic. Instantly will send them; no copy-out to another mail tool.':'Three cold emails: a short opener, one ownership question, then a close. Instantly will send them; no copy-out to another mail tool.';break;}
