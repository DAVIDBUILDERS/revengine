...
export async function operationalCommand(command:C.Command,requested:string|null):Promise<C.CommandResult>{
 ...
 switch(command.type){
  ...
  case 'generate_outbound_sequence':{const company=outboundSequenceCompany(current);const model=hostedColdSequenceModel(workspaceId,current.workspace.mode);const drafted=await writeOutboundSequence(company,current.outboundSdr?.bookingUrl??'',model,{requireModel:Boolean(model)});await rpc('save_outbound_sequence',{p_workspace:workspaceId,p_sequence:drafted.sequence});message=drafted.source==='model'?'Three cold emails drafted from your topic. Instantly will send them; no copy-out to another mail tool.':'Three cold emails drafted for this topic. Instantly will send them; no copy-out to another mail tool.';break;}
