...
import {parseCsvImport,forecastCases,parseLeadCsv,mappedLeadRow,generateOutboundSequence,companyFromSnapshot,emptyOutboundSdr,outboundSequenceCompany,slotAgentIds,technicalSeoAnswers,emptyTechnicalSeo} from '../../../packages/domain/src/index';
...
export async function operationalCommand(command:C.Command,requested:string|null):Promise<C.CommandResult>{
 ...
 switch(command.type){
  ...
  case 'generate_outbound_sequence':{const sequence=generateOutboundSequence(outboundSequenceCompany(current),current.outboundSdr?.bookingUrl??'');await rpc('save_outbound_sequence',{p_workspace:workspaceId,p_sequence:sequence});message='Three cold emails: a short opener, one ownership question, then a close. Instantly will send them; no copy-out to another mail tool.';break;}
