...
export async function operationalCommand(command:C.Command,requested:string|null):Promise<C.CommandResult>{
 ...
 switch(command.type){
  ...
  case 'generate_outbound_sequence':{const company=outboundSequenceCompany(current);const bookingUrl=current.outboundSdr?.bookingUrl??'';const model=current.workspace.mode==='fixture'?undefined:hostedColdSequenceModel(workspaceId,current.workspace.mode);const drafted=await writeOutboundSequence(company,bookingUrl,model);await rpc('save_outbound_sequence',{p_workspace:workspaceId,p_sequence:drafted.sequence});message=drafted.source==='model'?'Three cold emails drafted from your services with the cold-outreach prompt. Instantly will send them; no copy-out to another mail tool.':'Three cold emails drafted from the same cold-outreach rules. Instantly will send them; no copy-out to another mail tool.';break;}
