...
export function Conversation(props:ScreenProps){
 ...
 return <div className="briefing-shell">
  ...
  <main id="main-content" className="briefing-main">
   ...
   {step==='research'&&<Question id={step} title={b.noWebsite?'Let’s fill in just the essentials.':title[step]} description={b.noWebsite?'We’ll use the description you provided and ask only for the missing facts.':research==='running'?'The public pages are still being read. You can wait here or continue with your own answers.':research==='ready'?'We found explicit wording to propose. You’ll review it on the next screen.':research==='failed'?'We couldn’t read this site. You can continue with your own answers.':'No completed research is available for this website yet.'} onContinue={()=>void run(async()=>{useResearch();await forward();})} label={usableCapture?'Review what we found':'Continue with my own answers'}>
   ...
   {step==='company-name'&&<Question id={step} title={title[step]} onContinue={next} disabled={!a.company.name.trim()}>{field('Company name',a.company.name,v=>company('name',v))}</Question>}
   {step==='offer'&&<Question id={step} title={title[step]} onContinue={next} disabled={!a.company.offers.some(v=>v.trim())}>{field('Main offer',a.company.offers.join('\n'),v=>company('offers',v),true)}</Question>}
   {step==='customers'&&<Question id={step} title={title[step]} onContinue={next} disabled={!a.company.customers.some(v=>v.trim())}>{field('Ideal customers',a.company.customers.join('\n'),v=>company('customers',v),true)}</Question>}
   {step==='summary'&&<Question id={step} title={title[step]} description={usableCapture?'Suggested facts remain unconfirmed until you review them.':'Based on your answers. Website research has not verified these facts.'} onContinue={()=>void run(confirmSummary)} disabled={!a.company.name||!a.company.offers.length||!a.company.customers.length} label="That’s right">
    <div className="briefing-summary"><h2>{a.company.name}</h2><p>{a.company.offers.join(' · ')}</p><p className="muted">For {a.company.customers.join(' · ')}</p></div>
