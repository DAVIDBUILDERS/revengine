...
export function TeamStudio(props:ScreenProps & {configuration:ReactNode}){
 ...
 return <div className="team-studio">
  ...
  <nav className="studio-team" aria-label="Choose an agent">{agents.map((a,i)=>{const n=state.artifacts.filter(x=>x.agentId===a.id).length;return <button key={a.id} className={`studio-specialist ${agent?.id===a.id?'is-current':''}`} aria-pressed={agent?.id===a.id} onClick={()=>selectAgent(a.id)}><span className="studio-specialist-top"><span className="studio-index">0{i+1}</span><ArrowUpRight size={16}/></span><strong>{a.name}</strong><span>{n?`${n} saved output${n===1?'':'s'}`:a.releaseStatus==='planned'?'Capability planned':'Awaiting first work'}</span></button>})}<button className="studio-add" onClick={()=>(setBuilderOpened(true),setSettings(true))}><Plus size={23}/><span>{agents.length?'Edit team':'Choose your specialists'}</span></button></nav>
