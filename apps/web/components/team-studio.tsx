...
import {agentDelivery, specialistWorkSnapshot} from '@david/domain/delivery';
...
export function TeamStudio(props:ScreenProps & {configuration:ReactNode}){
 ...
 return <div className="team-studio">
  ...
  {agent?<section className="studio-workbench" aria-label={`${agent.name} workbench`}>
   <aside className="studio-brief"><span className="studio-kicker">{isIncludedAgent(agent.id)?'INCLUDED WITH EVERY WORKSPACE':'ASSIGNED SPECIALIST'}</span><h2>{agent.name}</h2><p>{agent.responsibility.replace(/source-grounded /g,'').replace(/WebsiteProfile/g,'company research').replace(/ContentBrief/g,'a content brief').replace(/CapturedPageAudit/g,'a website assessment')}</p>
   ...
   <button className="studio-text-action" onClick={setup}>{delivery?.mode==='engineering_required'||agent.releaseStatus==='planned'?'Record needed systems':delivery?.mode==='needs_access'?'Review source access':'Company sources'} <ArrowUpRight size={14}/></button><button className="studio-text-action" onClick={()=>setInspect(true)}><ShieldCheck size={15}/>Access, limits & history</button></div>
