...
Release verification: scoped lint and TypeScript passed; all 16 Google connector fixture tests passed. Test deployment `FXDMHErqHbUrCNCa8vXVtNYCEAfP` and production deployment `pqwLKv7JQBKP6B6NATyoQM2KHxcT` are ready. Production's canonical Google callback now passes configuration checks and returns 400 `VALIDATION_FAILED` for a request missing state/code, rather than 503 `GOOGLE_UNCONFIGURED`. Canonical login/start and auth/origin checks passed. This proves the configuration is loaded, not a completed Google account authorization or credential exchange.
